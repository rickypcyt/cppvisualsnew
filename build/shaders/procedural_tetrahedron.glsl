...
void pRR(inout vec2 p, float a) {
    p = cos(a)*p + sin(a)*vec2(p.y, -p.x);
}

float smaxxx(float a, float b, float r) {
...
float tetAnim(vec3 p, float time) {
    ...
    oct = smaxxx(oct, (dot(p, n2) - sz), rBase);
    oct = smaxxx(oct, -(dot(p, n4) + .5 - sep), rInner);
    oct = smaxxx(oct, -(dot(p, n3) + .1 - sep), rInner);
    oct = smaxxx(oct, (dot(p, n4) + .1 + sep), rInner);
    oct *= scale;
    ...
    edge = smaxxx(edge, (dot(p, n2) - sz), rBase);    
    
    // side tets
    p = pp + (n4 + n3) * offset;
    p /= scale;
    float side = edge;
    side = smaxxx(side, (dot(p, n3) + .1 + sep), rInner);
    side = smaxxx(side, (dot(p, n4) + .1 + sep), rInner);
    side *= scale;

    // vertex tets
    p = pp + n4 * (offset + offset);
    p /= scale;
    float vert = edge; vert = smaxxx(vert, (dot(p, n3) - sz), rBase);
    vert = smaxxx(vert, (dot(p, n4) + .5 + sep), rInner);
    vert *= scale;
    ...
    if (time < 1.) {
        ...
        inner = smaxxx(inner, -(dot(p, n3) + .1 - sep), rInner);
        inner = smaxxx(inner, -(dot(p, n2) + .1 - sep), rInner);
    ...
    base = smaxxx(base, (dot(p, n2) - sz), rBase);
    base = smaxxx(base, (dot(p, n3) - sz), rBase);
...
vec3 render(vec2 fragCoord, float iTime) {
    initPoly();
    ...
    for (float disperse = 0.; disperse < MAX_DISPERSE; disperse++) {
        ...
        // Simple noise instead of texture
        float rand = fract(sin(dot(fragCoord, vec2(12.9898, 78.233))) * 43758.5453);
        rand = fract(rand + iTime * 1.61803398875);
    ...
    // debug
 	//return spectrum(steps / 2000.);
    //return vec3(bounceCount / MAX_BOUNCE);
    //return vec3(firstHit.steps / 100.);
